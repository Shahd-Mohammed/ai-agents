...
def execute_read_query(db, sql):
    """
    Execute only safe SELECT queries generated by the Read Expert.
    """
...
def execute_write_action(db, generated_code):
    """
    Execute Python code generated by the Write Expert.
    """
...
def run_orchestrator_plan(db, original_request, plan_text):
    """
    Run the Orchestrator plan one expert at a time and in order.
    """
    ...
    if plan_text.startswith("```"):
        ...
        if plan_lines and plan_lines[-1].strip() == "```":
            plan_lines = plan_lines[:-1]

        plan_text = "\n".join(plan_lines)

    try:
        call_strings = ast.literal_eval(plan_text)
    except Exception as error:
        print(f"Orchestrator returned an unparseable plan: {plan_text}")
        print(f"Plan parsing error: {error}")
    ...
    for call_string in call_strings:
        ...
        if not match:
            print(f"Could not understand this plan step: {call_string}")
            continue
    ...
    if not results:
        return "Sorry, I couldn't execute that plan."

    steps_summary = "\n".join(
        f"{role}: {response}"
        for role, message, response in results
    )
    ...
    synthesis_prompt = (
        ...
        f"{steps_summary}\n\n"
        "Write one short and clear reply. "
        "If a Database Write Expert result is present, reuse its "
        "result message exactly. Otherwise summarize the results "
        "in plain language. Do not mention SQL, Python, code, "
        "the Orchestrator, or internal expert steps."
    )
...
def handle_ai_chat_request(db, role, message):
    """
    Send a request to the selected AI expert.
    """
    if role is None:
